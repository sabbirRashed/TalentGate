...
import { Table, Button, Avatar, toast } from '@heroui/react';
...
export default function CompanyTable({ companies = [] }) {
    ...
    const handleApprove = async(company) => {
        const result = await updateCompany(company._id, {status: 'Approved'})
        if(result.modifiedCount >0){
            toast.success(`Approved ${company.name} successfully`)
        }
        console.log('Approved:', company, result);
    
    };

    const handleReject = async(company) => {
        const result = await updateCompany(company._id, {status: 'Rejected'})
        console.log('Rejected:', company);
    };
                        ...
                        <Table.Body>
                            {companies.map((company) => {
                                ...
                                return (
                                    ...
                                    >
                                        {/* Company Name with Hero UI Avatar compound component */}
                                        <Table.Cell className="py-4 px-6">
                                            <div className="flex items-center gap-3">
                                                <Avatar className="w-8 h-8 rounded-lg bg-zinc-800 border border-zinc-700/60 text-xs font-bold text-zinc-200 shrink-0">
                                                    {company.logo && (
                                                        <Avatar.Image
                                                            src={company.logo}
                                                            alt={company.name}
                                            ...
                                            <div className="flex items-center justify-end gap-2">
                                                {!isApproved && (
                                                    <Button
                                                        ...
                                                        className="px-3 py-1 text-xs font-medium text-emerald-400 bg-emerald-950/30 border border-emerald-800/60 hover:bg-emerald-900/50 rounded-md min-w-0 h-auto"
                                                    >
                                                        Approve
                                                    </Button>
                                                )}
                                                {!isRejected && (
                                                    <Button
                                                        ...
                                                        className="px-3 py-1 text-xs font-medium text-rose-400 bg-rose-950/30 border border-rose-800/60 hover:bg-rose-900/50 rounded-md min-w-0 h-auto"
                                                    >
                                                        Reject
                                                    </Button>
                                                )}
